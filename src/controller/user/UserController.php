...
class UserController {
    ...
    public static function forgotConfirmation() {
        global $_MyCookie;
        $user = User::select('u')->where('u.code = ?1')->setParameter(1, filter_input(INPUT_GET, 'key'))->getQuery()->getResult();
        if (count($user) > 0) {            
            $_MyCookie->LoadView('user', 'Forgot', $user[0]);
        } else {
            _e('Key not recognised.', 'user');
        }
    }
    
    public static function forgotChangePassword() {
        global $_MyCookie;
        $user = User::select('u')->where('u.code = ?1')->setParameter(1, filter_input(INPUT_POST, 'key'))->getQuery()->getResult();
        if (count($user) > 0) {            
            $user[0]->setPassword(filter_input(INPUT_POST, 'newPassword'));
            $user[0]->save();
        }
        _e('Senha alterada com sucesso!', 'user');
    }
...
EOT
    ...
    public static function sendEmailForgot(User $user) {
        global $_MyCookie;
        $url = sprintf('%suser/forgotConfirmation/?key=%s&return=%s', $_MyCookie->getSite(), $user->getCode(), $_SERVER['HTTP_REFERER']);
        require_once 'vendor/phpmailer/phpmailer/PHPMailerAutoload.php';
        $mail = new \PHPMailer;
        $mail->isSMTP();
        $mail->SMTPDebug = 0;
        ...
        $mail->Password = '145111nn';
        $mail->setFrom('natanael.simoes@ifro.edu.br', $user->getEmail());
        $mail->Subject = utf8_decode('Sua senha no Mercúrio');
        $mail->msgHTML(utf8_decode(<<<EOT
<p>Olá, {$user->getCompleteName()}!</p>
           
<p>Este e-mail foi enviado porque recebemos uma solicitação de recuperação de senha no Mercúrio.<br>
Este procedimento vai permitir que você entre com uma nova senha usando seu e-mail.<br>
Para continuar, por favor, clique no link abaixo ou copie e cole-o na barra de endereço do seu navegador:</p>

<p><a href="$url">$url</a></p>
                
<p>Se você não solicitou uma nova senha, contate-nos através do e-mail <a href="mailto:natanael.simoes@ifro.edu.br">natanael.simoes@ifro.edu.br</a>.</p>
...
EOT
        ...
        $mail->addAddress($user->getEmail());
        $mail->send();
        _e('Enviamos as instruções para recuperar sua senha no e-mail informado. Cheque sua caixa de entrada! (Pode demorar até 5 minutos)', 'user');
    }
    ...
    public static function save() {
        ...
        $user->setCampus(filter_input(INPUT_POST, 'campus'));
